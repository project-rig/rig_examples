...
def create_netlist(mc, n_transmitters=3):
    # Create objects to represent the transmitters and the receiver
    transmitters = [object() for _ in range(n_transmitters)]
    receiver = object()

    # Create a dictionary mapping each object ("vertex") to the application
    # that it represents.
    vertices_applications = {t: "transmitter.aplx" for t in transmitters}
    vertices_applications[receiver] = "receiver.aplx"

    # Indicate that each application requires 1 processing core. In addition,
    # each transmitter requires 2 words of SDRAM and the receiver 1 word.
    ...
    return (transmitters, receiver, placements, allocations,
            application_map, routing_tables)


def load_and_run(mc, transmitters, receiver, placements, allocations,
                 application_map, routing_tables):
    with mc.application():
...
if __name__ == "__main__":
    # Create a new MachineController for the SpiNNaker machine whose hostname
    # is given on the command line
    mc = MachineController(sys.argv[1])

    # Create the netlist to simulate, place and route it
    (transmitters, receiver, placements, allocations, application_map,
     routing_tables) = create_netlist(mc)

    # Load and run the netlist
    load_and_run(mc, transmitters, receiver, placements, allocations,
                 application_map, routing_tables)
